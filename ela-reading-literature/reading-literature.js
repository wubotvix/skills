/**
 * AutoClaw ELA Reading Literature Progress Tracker
 * No external dependencies. Node.js built-in APIs only.
 *
 * Usage:
 *   const ReadingLiterature = require('./reading-literature');
 *   const rl = new ReadingLiterature();
 *   const profile = await rl.getProfile('student-1');
 *   await rl.recordAssessment('student-1', 'grade-2-3', 'character', 'describe-traits', 4, 5);
 *   const progress = await rl.getProgress('student-1');
 *   const next = await rl.getNextSkills('student-1');
 *   const report = await rl.getReport('student-1');
 */
...
const DATA_DIR = path.join(AGENT_DIR, 'data', 'ela-reading-literature');
...
const SKILLS = {
  'k-1': {
    'characters': ['name-characters', 'describe-characters'],
    'setting': ['identify-setting', 'describe-setting-importance'],
    'events': ['retell-bme', 'retell-key-details-in-order'],
    'problem-solution': ['identify-problem', 'describe-problem-solution'],
    'illustrations': ['use-pictures', 'illustrations-add-to-story'],
    'connections': ['text-to-self', 'text-to-text'],
  },
  'grade-2-3': {
    'character': ['respond-to-events', 'describe-traits-motivations-feelings'],
    'theme': ['identify-lesson-fable', 'determine-theme-with-details'],
    'point-of-view': ['character-viewpoints', 'own-vs-narrator-pov'],
    'text-structure': ['beginning-middle-end', 'chapters-scenes-stanzas'],
    'vocabulary': ['context-clues', 'literal-vs-figurative'],
    'comparing': ['compare-versions-same-story', 'compare-themes-same-author'],
  },
  'grade-4-5': {
    'theme': ['determine-theme-summarize', 'two-or-more-themes'],
    'character': ['describe-in-depth-with-evidence', 'compare-contrast-characters'],
    'point-of-view': ['first-vs-third-person', 'narrator-pov-influences'],
    'figurative-language': ['simile-metaphor-idiom', 'personification-hyperbole'],
    'structure': ['chapters-stanzas-fit-together', 'scenes-provide-structure'],
    'comparing-texts': ['compare-themes-across-genres', 'compare-across-cultures'],
  },
  'grade-6': {
    'theme': ['analyze-theme-development'],
    'character': ['analyze-character-change'],
    'pov': ['explain-author-develops-pov'],
    'figurative-language': ['analyze-word-choice-impact'],
    'structure': ['analyze-sentence-chapter-scene-fit'],
    'comparing': ['compare-genres-approaches'],
  },
};
