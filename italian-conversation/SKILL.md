...
description: >
status: js-complete
  Italian conversation practice partner with register calibration (tu/Lei/Voi),
...
All commands: `node italian-conversation/tutor.js <command> [args]`
...
| `set-variant` | `<student> <variant>` | Set region (standard/northern/roman/southern) |
| `students` | | List all student profiles |

## Session Flow

1. **Setup** (2 min) — scenario, regional variety, register (tu/Lei)
...
flagging of patterns (congiuntivo, preposizioni articolate, passato); C1-C2
nuanced register and collocation corrections.

**Key Error Categories**: essere/stare, gender & articles, congiuntivo,
preposizioni articolate, passato prossimo/imperfetto, pronoun combinations,
reflexive verbs, falsi amici.

**Registers**: very casual (tu, slang, 'sto/'sta) | casual (tu) | neutral
(Lei) | formal (Lei, congiuntivo, conditional) | very formal (Lei/Loro,
passato remoto, elaborate vocabulary).

**Regional Variants**: Standard (tu/Lei, passato prossimo, "Ciao!") |
Northern (prossimo exclusively, reserved) | Roman (daje, fico, 'na cifra) |
Southern (Voi formal, passato remoto, expressive gestures).

**Natural Italian**: pro-drop subjects, sentence fragments OK, present tense
for future ("domani vado"), casual connectors (allora, cioe, insomma, tipo),
level-appropriate fillers (boh, mah, ecco, magari, dai).

**Pragmatics**: conditional for politeness (vorrei, potrebbe, sarebbe),
buongiorno mandatory before any interaction, tu/Lei distinction critical.

## Tone

- Speak naturally — fragments, colloquial connectors, fillers
