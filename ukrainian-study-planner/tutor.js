...
const SKILL_NAME = 'ukrainian-study-planner';
...
const GOAL_ALLOCATIONS = {
  ...
  heritage:     { vocabulary: 20, grammar: 15, conversation: 25, pronunciation: 10, reading: 15, writing: 5,  listening: 10 },
  literary:     { vocabulary: 25, grammar: 20, conversation: 5,  pronunciation: 5,  reading: 30, writing: 10, listening: 5 },
};
...
const PLACEMENT_QUESTIONS = [
  {
    level: 'A1',
    topic: 'Nominative — basic sentence',
    prompt: 'Доповніть: "Це ___." (a) книга (b) книги (c) книзі',
    options: ['книга', 'книги', 'книзі'],
    answer: 0,
    hints: [
      'Think about which case is used after "це" (this is).',
      'After "це" you need the nominative case.',
      'It is either (a) or (b).',
    ],
  },
  {
    ...
    prompt: 'Доповніть: "Вчора вона ___ додому." (a) ішов (b) ішла (c) йти',
    options: ['ішов', 'ішла', 'йти'],
    answer: 1,
    hints: [
      '"Вчора" tells you the tense. "Вона" tells you the gender.',
      'Past tense for feminine subjects ends in -ла.',
      'It is either (a) or (b).',
    ],
  },
  {
    level: 'B1',
    topic: 'Genitive after немає',
    prompt: 'Доповніть: "У мене немає ___." (a) книга (b) книги (c) книзі',
    options: ['книга', 'книги', 'книзі'],
    answer: 1,
    hints: [
      '"Немає" requires a specific case. Which one?',
      'After "немає" you need the genitive case.',
      'It is either (a) or (b).',
    ],
  },
  {
    level: 'B2',
    topic: 'Vocative case',
    prompt: 'Доповніть: "___, ходи сюди!" (a) Олена (b) Олено (c) Олені',
    options: ['Олена', 'Олено', 'Олені'],
    answer: 1,
    hints: [
      'This is direct address. Which case do you use for calling someone?',
      'Ukrainian uses the vocative case for direct address. Feminine names ending in -а change to -о.',
      'It is either (a) or (b).',
    ],
  },
  {
    level: 'C1',
    topic: 'Synthetic future',
    prompt: 'Доповніть: "Я ___ це завтра." (a) робитиму (b) роблю (c) робив',
    options: ['робитиму', 'роблю', 'робив'],
    answer: 0,
    hints: [
      '"Завтра" tells you the tense. Ukrainian has a synthetic future form.',
      'The synthetic future adds -тиму/-тимеш to the infinitive stem.',
      'It is either (a) or (b).',
    ],
  },
];
...
class StudyPlanner {
  ...
  getReport(id) {
    ...
    return {
      ...dash,
      report: {
        currentMilestones: milestones.map(m => ({ achieved: lvl, en: m.en, uk: m.uk })),
        nextLevel: nextLvl || 'max',
        nextMilestones: nextMilestones.map(m => ({ target: nextLvl, en: m.en, uk: m.uk })),
...
core.runCLI((cmd, args, out) => {
  switch (cmd) {
    ...
    case 'set-goal': {
      const [, id, goal] = args;
      if (!id || !goal) throw new Error('Usage: set-goal <student> <conversation|work|exam|travel|academic|heritage|literary>');
