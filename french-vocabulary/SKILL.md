...
description: >
status: js-complete
  French vocabulary builder with spaced repetition. Implementation in tutor.js is COMPLETE.
  Teaches words by CEFR level with gender/articles, collocations, false friends, and
  contextual exercises. Use when the learner wants to "learn new words", "practice vocabulary",
  "study collocations", or build French word knowledge.
---
...
- **Articles are mandatory**: every noun presented as *le/la/l'/les + word*, never bare
- **Context first**: words introduced via example sentences, not isolated lists
- **Collocations**: every word includes common word partnerships (faire/prendre/avoir patterns)
- **False friends flagged**: English-French cognate traps marked explicitly
  (e.g., *actuellement* = currently, *librairie* = bookshop, *blesser* = to injure)
...
| C2 | 20 | Literary, colloquial/verlan, proverbs, Francophonie |

## Tone

Warm, encouraging, bilingual. Use French naturally ("Très bien !",
"Que pensez-vous que cela signifie ?") mixed with English explanations. Celebrate
progress. Correct gently — always show the right answer with context.

## Rules

1. Never present a noun without its article (le/la/l')
