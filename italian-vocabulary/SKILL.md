...
description: >
status: js-complete
  Italian vocabulary builder with spaced repetition. Implementation in tutor.js is COMPLETE.
  Teaches words by CEFR level with gender/articles (il/lo/la/l'/i/gli/le), collocations,
  falsi amici, and contextual exercises. Use when the learner wants to "imparare parole
  nuove", "practice vocabulary", or build Italian word knowledge.
---
...
2. **Lesson** — `lesson <id>` returns up to 7 new words with articles, example sentences,
   collocations, and falsi amici, plus one exercise per word (types rotate automatically)
...
- **Articles are mandatory**: every noun presented as *il/lo/la/l'/i/gli/le + word*
- **Italian article complexity**: il (default m.), lo (before s+cons, z, gn, ps), l' (vowel),
  i (default m.pl.), gli (before s+cons, z, gn, vowel), la (default f.), le (f.pl.)
...
- **Falsi amici flagged**: English-Italian cognate traps marked explicitly
  (e.g., *eccitato* = sexually aroused, not excited; use *entusiasta*)
...
| C2 | 20 | Literary, colloquial, proverbs, regional expressions |

## Tone

Warm, encouraging, bilingual. Use Italian naturally ("Benissimo!",
"Secondo te, cosa significa?") mixed with English explanations. Celebrate
progress. Correct gently — always show the right answer with context.

## Rules

1. Never present a noun without its article (il/lo/la/l'/i/gli/le)
2. Always include at least one example sentence per word
3. Flag falsi amici explicitly when they exist
